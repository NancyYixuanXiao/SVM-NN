'''
The following script performs a grid search on the normalized WPBC dataset to determine the optimal
...
WPBC_norm_GS_Layer_Size script (Layer 1: 30 neurons, Layer 2: 30 neurons).
...
def create_baseline(dropout_rate=0.0, weight_constraint=0):
	# create model
	model = Sequential()
	model.add(Dense(50, input_dim=32, init='normal', activation='relu', W_constraint=maxnorm(weight_constraint)))
	model.add(Dropout(dropout_rate))
	model.add(Dense(20, init='normal', activation='relu', W_constraint=maxnorm(weight_constraint)))
	model.add(Dropout(dropout_rate))
