...
Please note this script takes approximtaely 9 hours to run on AWS GPU.
Top Classification Accuracy: 98.07%
'''
...
def create_baseline(layer1=1,layer2=1,dropout_rate=0.0, weight_constraint=0):
	# create model
	model = Sequential()
	model.add(Dense(layer1, input_dim=30, init='normal', activation='relu', W_constraint=maxnorm(weight_constraint)))
	model.add(Dropout(dropout_rate))
	model.add(Dense(layer2, init='normal', activation='relu', W_constraint=maxnorm(weight_constraint)))
	model.add(Dropout(dropout_rate))
...
param_grid = dict(layer1=layer1,layer2=layer2,dropout_rate=dropout_rate, weight_constraint=weight_constraint)
