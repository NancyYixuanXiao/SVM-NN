'''
The following script performs a grid search on the normalized and degraded WPBC dataset to determine the optimal size for a
neural network with two hidden layers.  The optimal size is determine by the neural
network with the maximum classifcation accuracy (recall).  Layers sizes between 30 and 90 neurons
...
dataset = numpy.loadtxt("WPBC_norm_degraded.csv", delimiter=",")
...
def create_baseline(layer1 = 1, layer2 = 1):
	# create model
	model = Sequential()
	model.add(Dense(layer1, input_dim=32, init='normal', activation='relu'))
...
wrt = open("results_WPBC_norm_no_reg.txt", 'w')
