function svm_models(feature,label)
% all models
...
for model = 1:6
    ...
    if model == 1
        ...
        parfor i = 1:worker
            [CCR_model_1(i,:)] = svm_func(kfold,'linear','SMO',0,0,c_iter(i,:),1,feature,label);
        end
    ...
    if model == 2
        ...
        parfor i = 1:worker
            [CCR_model_2i(i,:,:)] = svm_func(kfold,'rbf','SMO',0,sigma_iter(i,:),c_iter(i,:),...
                1,feature,label);
        end
...
%         disp(sprinf('L1 norm polynomial kernal complete'));
    ...
    if model == 4
        ...
        %
        %     parfor i = 1:worker
        %         [CCR_model_4(i,:)] = svm_func(kfold,'linear','SMO',0,0,c_iter(i,:),1.5,feature,label);
        %     end
...
%         disp(sprinf('L1 norm linear kernal with non-uniform cost complete'));
    ...
    if model == 5
        ...
        parfor i = 1:worker
            [CCR_model_5(i,:)] = svm_func(kfold,'linear','QP',0,0,c_iter(i,:),1,feature,label);
        end
