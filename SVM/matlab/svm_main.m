...
numofpoly = 24; % start at 2^(numofpoly-4), number of polynomial degree to cross validate
...
plot(maxC_CCR_lin-10,CCRlin(1,maxC_CCR_lin),'x','MarkerSize',40);
...
plot(maxC_CCR_sig-10,maxsigma_CCR-2,'x','MarkerSize',20); hold off
...
plot(max(maxC_prec_sig)-10,max(mm)-2,'x','MarkerSize',20); hold off
...
plot(max(maxC_rec_sig)-10,min(mk)-2,'x','MarkerSize',20); hold off
subplot(2,2,4); contourf(X,Y,fscorerbf); colorbar; title('f-score');hold on
ylabel('Sigma = 2^s'); xlabel('C = 2^c'); hold on
plot(maxC_fsc_sig-10,maxsigma_fsc-2,'x','MarkerSize',20); hold off
